...
public class IntrantThirdPersonMotor : IntrantPlayerBasicBehaviour
{
    ...
    {
        //Place condition of movement to be done only when not attacking 
        if (!attacking)
        {
    ...
    protected virtual void CheckGroundDistance()
    {
        if (_capsuleCollider != null)
        {
            ...
            dist = 10f;
            // ray for RayCast
            ray2 = new Ray(transform.position + new Vector3(0, colliderHeight / 2, 0), Vector3.down);
            ...
            {
                pos = transform.position + Vector3.up * (_capsuleCollider.radius);
                ...
                if (Physics.SphereCast(ray, radius, out groundHit, _capsuleCollider.radius + groundMaxDistance, groundLayer) && !groundHit.collider.isTrigger)
                {
                    ...
                    newDist = transform.position.y - groundHit.point.y;
